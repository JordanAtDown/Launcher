...
pub fn restore(cfg: &UpdatesConfig) -> bool {
        ...
        .status()
    {
        ...
        Ok(s) if s.code() == Some(2) => true, // déjà démarré — état désiré atteint
        Ok(s)  => { log::warn!("updates: wuauserv start exit={:?}", s.code()); false }
